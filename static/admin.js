...
function flag_in_suggestions(SmObjId, PiqYear, PiqSession, whitelisted){
    var tr_module = $("#suggestions").find(`[data-smobjid='${SmObjId}'][data-semester='${PiqYear} ${PiqSession.toString().padStart(3, 0)}']`);
...
function write_tr_prefix_for_list(module_id, SmObjId, PiqYear, PiqSession, title, searchterm, searchterm_id){
    ...
    return `<tr id="module_${module_id}" data-SmObjId="${SmObjId}" data-semester="${PiqYear} ${PiqSession.toString().padStart(3, 0)}" class="shown">
...
async function populate_whitelist(){
    ...
    await $.ajax({
        ...
        error : function (err) {
            console.warn('Whitelist konnte nicht abgerufen werden: ', err.status, err); return;
        },
        complete : function() {
            // hide loading screen
            $('#whitelist').find('div.loading-overlay').toggle();
...
async function populate_blacklist(){
    ...
    await $.ajax({
        ...
        error : function (err) {
            console.warn('Blacklist konnte nicht abgerufen werden: ', err.status, err); return;
        },
        complete : function() {
            // hide loading screen
            $('#blacklist').find('div.loading-overlay').toggle();
...
async function populate_suggestions(){
    ...
    await $.ajax({
        ...
        error : function (err) {
            console.warn('Suchvorschläge konnten nicht abgerufen werden: ', err.status, err); return;
        },
        complete : function() {
            // hide loading screen
            $('#suggestions').find('div.loading-overlay').toggle();
...
async function populate_studyprograms() {
    ...
    await $.ajax({
        ...
        error : function (err) {
            console.warn('/studyprograms konnten nicht abgerufen werden: ', err.status, err); return;
        },
        complete : function() {
    ...
    await $.ajax({
        ...
        error : function (err) {
            console.warn('/studyprograms_modules konnte nicht abgerufen werden: ', err.status, err); return;
        },
    })
